...
$hashTable = @{
    "\\mailrelayx\c$\Program Files (x86)\Mail Enable\Logging\.*.log" = 30
}
...
foreach ($key in $keyArray)
{
    ...
    $files = get-childitem $folder -Recurse | where {$_.Name -imatch $pattern} | where { $_.LastWriteTime -lt $DatetoDelete } | Remove-item -Force -whatif
}
